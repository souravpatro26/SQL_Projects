...
select top 1 state, sum(quantity)
from DIM_LOCATION L
inner join fact_transactions T on L.IDLocation =T.IDlocation
inner join dim_model M on T.IDmodel = M.IDmodel
...
ORDER BY State DESC,No_of_Transactions DESC
...
select Manufacturer_Name,Model_name,sum(quantity) as Quantity_sold,Avg(unit_price) Avg_Unit_Price 
from dim_model M
inner join dim_manufacturer F on M.IDmanufacturer =F.IDmanufacturer
...
order by sum(quantity) desc,Avg(unit_price) desc)
group by Manufacturer_Name,Model_name
order by Manufacturer_Name,Quantity_sold desc
...
select top 5 Model_Name,sum(quantity) as Quantity,year(t.Date) as Year
from DIM_MODEL m
join FACT_TRANSACTIONS t on m.IDModel=t.IDModel
where Year(date)=2008
group by Model_Name,year(t.Date)
...
select * from
(select year(date) as year_of_sale, Manufacturer_Name,sum(T.TotalPrice) as Total_2009,RANK() over(order by sum(T.TotalPrice) desc) as Rank_2009
from DIM_MANUFACTURER F
left join DIM_MODEL M on F.IDManufacturer=M.IDManufacturer
left join FACT_TRANSACTIONS T on M.IDModel=T.IDModel
where YEAR(Date)=2009 
...
select * from
(
select year(date) as year_of_sale, Manufacturer_Name,sum(T.TotalPrice) as Total_2010,RANK() over(order by sum(T.TotalPrice) desc) as Rank_2010
from DIM_MANUFACTURER F
left join DIM_MODEL M on F.IDManufacturer=M.IDManufacturer
left join FACT_TRANSACTIONS T on M.IDModel=T.IDModel
where YEAR(Date)=2010 
